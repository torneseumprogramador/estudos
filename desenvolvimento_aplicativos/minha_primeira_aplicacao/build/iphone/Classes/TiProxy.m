...
//Rare exceptions to indicate a bug in the _minha_primeira_aplicacao code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_minha_primeira_aplicacao = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_minha_primeira_aplicacao]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.minha_primeira_aplicacao.module"
