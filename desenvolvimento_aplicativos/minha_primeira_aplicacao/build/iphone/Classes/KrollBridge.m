...
NSString * minha_primeira_aplicacao$ModuleRequireFormat = @"(function(exports){"
...
@implementation minha_primeira_aplicacaoObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_minha_primeira_aplicacao);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create minha_primeira_aplicacao global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "minha_primeira_aplicacao" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_minha_primeira_aplicacao = [[minha_primeira_aplicacaoObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_minha_primeira_aplicacao];
	
	NSString *_minha_primeira_aplicacaoNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _minha_primeira_aplicacaoNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_minha_primeira_aplicacao valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:minha_primeira_aplicacao$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.minha_primeira_aplicacao.kroll" 
